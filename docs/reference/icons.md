---
title: EspControl Icon Reference
description: Browse all available icons for your EspControl cards.
---

# Icon Reference

Click any icon to copy its name to the clipboard, then paste it into the icon field on your panel's web page.

<IconGallery />

## Missing an Icon?

If the icon you need isn't in the list, [open an issue](https://github.com/emolenev/espcontrol/issues) with the icon name and what you'd use it for, and we'll look into adding it.
